...
void displayReservedSeats(Seat seats[ROWS][COLUMNS], char *category);
...
void displayReservedSeats(Seat seats[ROWS][COLUMNS], char *category) {
    printf("Reserved seats for category %s:\n", category);
    for (int i = 0; i < ROWS; i++) {
        for (int j = 0; j < COLUMNS; j++) {
            if (strcmp(seats[i][j].category, category) == 0 && (seats[i][j].status == '#' || seats[i][j].status == 'o')) {
                printf("%c%d ", 'A' + i, j + 1);
            }
        }
    }
...
void reserveSeats(Seat seats[ROWS][COLUMNS], char *showID) {
    ...
    displayReservedSeats(seats, category);
    ...
    while (seatToken != NULL) {
        ...
        if (row >= 0 && row < ROWS && col >= 0 && col < COLUMNS) {
            if (action == '#' && seats[row][col].status == '*') {
                seats[row][col].status = '#';
                printf("Seat %s confirmed as reserved with pay\n", seatToken);
            } else if (action == 'o' && seats[row][col].status == '*') {
                ...
                printf("The seat %s is already reserved. Do you want to edit (Y/N): ", seatToken);
                scanf(" %c", &proceedEdit);
                getchar();
                if (proceedEdit == 'Y') {
                    if (action == '#') {
